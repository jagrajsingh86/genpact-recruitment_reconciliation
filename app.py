...
from engine.io_excel import read_config, reconcile_workbooks, register_to_bytes
from engine.library import run_library
from engine.reconcile import ReconciliationConfig, ReconciliationResult
...
st.markdown(
    ...
    .uploaded-banner {background: #FFAD28; color: #181C23;}
...
def render_kpis(result: ReconciliationResult) -> None:
...
def render_register_panel(
    ...
    *,
    ...
    severity_options = [
        severity for severity in ("HIGH", "MEDIUM", "LOW") if severity in set(frame.get("Severity", []))
    ]
    status_options = [
        status for status in ("New", "Recurring") if status in set(frame.get("Status", []))
    ]
    ...
    recruiters = filter_columns[2].multiselect(
        "Recruiter",
        recruiter_options,
    ...
    search_text = filter_columns[3].text_input(
        "Search",
        placeholder="Requisition, field, value…",
    ...
    st.download_button(
        ...
        data=register_to_bytes(result, data_mode=data_mode),
        file_name=f"RCM_EC_Register_{run_date}.xlsx",
    ...
    st.markdown(
        '<div class="resolved-heading">✓ Resolved since previous run</div>',
        unsafe_allow_html=True,
    )
    if result.resolved:
        st.dataframe(pd.DataFrame(result.resolved), use_container_width=True, hide_index=True)
    else:
        st.success("No issues were resolved since the previous run.")


def render_digest_panel(result: ReconciliationResult, run_date: str, *, key_prefix: str) -> None:
    ...
    digest_names = tuple(result.digests)
    digest_labels = {
        name: f"{name} — {result.digests[name].count} items ({result.digests[name].high_count} HIGH)"
    ...
    selected_recruiter = st.selectbox(
        ...
        key=f"{key_prefix}_digest_recruiter",
    )
    digest = result.digests[selected_recruiter]
    subject = f"RCM–EC reconciliation: {digest.count} items need your review ({run_date})"
    st.markdown(
...
def render_config_panel(
    ...
    *,
    ...
    country_false_positives = sum(
        finding["Field"] == "Country <> Country" for finding in result.findings
    )
    if allow_normaliser_toggle:
        ...
        st.success(
            f"ValueNormalisers applied — {country_false_positives} Country false positives in this run."
        )
...
def render_result(
    ...
    *,
    ...
    with register_tab:
        render_register_panel(result, run_date, data_mode=data_mode, key_prefix=key_prefix)
    with digests_tab:
        render_digest_panel(result, run_date, key_prefix=key_prefix)
    with config_tab:
        render_config_panel(
            config,
            result,
            allow_normaliser_toggle=allow_normaliser_toggle,
        )


def upload_signature(uploaded_file) -> tuple[str, int, str] | None:
    if uploaded_file is None:
        return None
    digest = hashlib.sha256(uploaded_file.getvalue()).hexdigest()
    return uploaded_file.name, uploaded_file.size, digest
...
if st.session_state.app_mode == "Library demo":
    try:
    ...
    render_result(
        results[selected_date],
        selected_date,
        config,
        data_mode="library",
        key_prefix="library",
        allow_normaliser_toggle=True,
    )
else:
    st.subheader("Run uploaded extracts")
    st.caption(
        "Files are processed in memory for this run. Requisition and position extracts are required; "
        "the previous register and runtime config are optional."
    )

    required_columns = st.columns(2)
    requisition_upload = required_columns[0].file_uploader(
        "Requisition report (.xlsx)",
        type=("xlsx",),
        key="upload_requisition",
    )
    position_upload = required_columns[1].file_uploader(
        "Position report (.xlsx)",
        type=("xlsx",),
        key="upload_position",
    )
    optional_columns = st.columns(2)
    previous_upload = optional_columns[0].file_uploader(
        "Previous register (.xlsx) — optional",
        type=("xlsx",),
        key="upload_previous",
    )
    config_upload = optional_columns[1].file_uploader(
        "Config workbook (.xlsx) — optional",
        type=("xlsx",),
        key="upload_config",
        help="Uses the bundled config when omitted.",
    )
    upload_run_date = st.date_input("Run date", value=date.today(), key="upload_run_date")
    current_signature = (
        upload_signature(requisition_upload),
        upload_signature(position_upload),
        upload_signature(previous_upload),
        upload_signature(config_upload),
        upload_run_date.isoformat(),
    )

    if st.button("Run reconciliation", type="primary", key="run_upload_reconciliation"):
        if requisition_upload is None or position_upload is None:
            st.session_state.pop("completed_upload_run", None)
            st.error("Reconciliation halted: both requisition and position reports are required.")
        else:
            try:
                for source in (
                    requisition_upload,
                    ...
                    if source is not None:
                        source.seek(0)
                config_source = config_upload if config_upload is not None else CONFIG_PATH
                with st.spinner("Running deterministic reconciliation…"):
                    upload_result = reconcile_workbooks(
                        requisition_upload,
                        position_upload,
                        config_source,
                        upload_run_date.isoformat(),
                ...
                if config_upload is not None:
                    config_upload.seek(0)
                upload_config = read_config(config_source)
                st.session_state.completed_upload_run = {
    ...
    completed_upload_run = st.session_state.get("completed_upload_run")
    if completed_upload_run and completed_upload_run["signature"] == current_signature:
        st.success("Reconciliation complete. Downloadable register ready below.")
        render_result(
            completed_upload_run["result"],
            upload_run_date.isoformat(),
            completed_upload_run["config"],
            data_mode="upload",
            key_prefix="upload",
            allow_normaliser_toggle=False,
        )
    elif requisition_upload is not None and position_upload is not None:
        st.info("Required extracts are ready. Select Run reconciliation to process them.")
    else:
        st.info("Upload both required extracts to begin. Optional files can be added at any time.")
